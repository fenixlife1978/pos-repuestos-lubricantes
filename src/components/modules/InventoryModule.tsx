...
import { Plus, Search, Edit2, PackagePlus, Trash2, FileText, BarChart3, History, Gift, Boxes, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="tabs flex border-b border-[#2a2a2a] overflow-x-auto no-print">
        ...
        <button onClick={() => setActiveTab('reporte_general')} className={`tab ${activeTab === 'reporte_general' ? 'active' : ''}`}>Reporte General</button>
        ...
        <ModalAjuste 
          ...
          onSave={(mov) => {
            const nuevosProds = state.productos.map(p => 
              p.id === mov.productoId ? { ...p, stock: mov.stockDespues } : p
            );
            updateState({ 
...
function ReporteGeneral({ state }: { state: AppState }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
      ...
      <div className="card">
        <div className="card-head"><h3>Resumen por Categoría</h3></div>
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
              ...
              {Array.from(new Set(state.productos.map(p => p.categoria))).map(cat => {
                ...
                return (
                  <tr key={cat}>
                    ...
                    <td>{catProds.reduce((s, p) => s + p.stock, 0)}</td>
...
function HistorialAjustes({ state }: { state: AppState }) {
  ...
  return (
    <div className="card animate-in fade-in slide-in-from-bottom-2">
      <div className="card-head"><h3>Historial de Ajustes</h3></div>
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
            ...
            {ajustes.map(m => {
              ...
              return (
                <tr key={m.id}>
                  ...
                  <td><span className={`badge ${m.tipo.includes('entrada') ? 'badge-ok' : 'badge-err'}`}>{m.tipo}</span></td>
...
function ModalAjuste({ producto, onClose, onSave }: { producto: Product, onClose: () => void, onSave: (m: Movimiento) => void }) {
    ...
    <div className="modal show">
      ...
      <div className="modal-box">
        ...
        <div className="modal-body space-y-4">
          <p className="text-sm opacity-60">Stock actual: <strong className="text-[#c8952e]">{producto.stock} unidades</strong></p>
