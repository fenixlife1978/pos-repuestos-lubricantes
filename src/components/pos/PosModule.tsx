...
export default function PosModule() {
  ...
  // ============================================
  // MANEJADORES DE CRÉDITO - AHORA USA EL COMPONENTE IMPORTADO
  // ============================================
  ...
  const handleCreditConfirm = (customer: Customer, amount: number) => {
    ...
    const updatedCustomers = (store.clientes || []).map((c: Customer) => 
      c.id === customer.id ? { ...c, debt: (c.debt || 0) + total } : c
    );
    ...
    toast({ 
      ...
      description: `Se cargaron ${formatUsd(total)} al cliente ${customer.name}.` 
    });
  ...
  return (
    <div className="h-screen bg-white flex flex-col font-sans">
      ...
      {/* ============================================================ */}
      {/* MODAL DE CRÉDITO - USANDO EL COMPONENTE IMPORTADO */}
      {/* ============================================================ */}
      <LoadCreditModal
