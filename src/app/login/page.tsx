  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#D4C5A6] via-[#C8B99A] to-[#B8A98A] flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-[440px] bg-white rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.25)] p-12 animate-in fade-in zoom-in duration-500">
        ...
        <div className="mb-10 text-center sm:text-left">
          <h1 className="text-[34px] font-extrabold text-black leading-tight mb-2 tracking-tight">¡Bienvenido!</h1>
          <p className="text-[#9CA3AF] text-[15px] font-medium">Ingresa tus credenciales para acceder al panel administrativo.</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative group">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] group-focus-within:text-[#C8952E] transition-colors"><User className="w-5 h-5" /></div>
              <select className="w-full h-[56px] bg-[#F9FAFB] border border-[#E5E7EB] rounded-2xl pl-12 pr-10 text-black font-semibold appearance-none focus:border-[#C8952E] focus:bg-white outline-none transition-all cursor-pointer" value={role} onChange={e => setRole(e.target.value)} required>
                <option value="" disabled>Selecciona tu rol de acceso</option>
                <option value="administrador">Administrador del Sistema</option>
                <option value="cajero">Cajero / Operador</option>
              </select>
              <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] pointer-events-none"><ChevronDown className="w-5 h-5" /></div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              ...
              <input type="email" required className="w-full h-[56px] bg-[#F9FAFB] border border-[#E5E7EB] rounded-2xl pl-12 pr-4 text-black font-semibold placeholder:text-[#D1D5DB] focus:border-[#C8952E] focus:bg-white outline-none transition-all" placeholder="ejemplo@correo.com" value={email} onChange={e => setEmail(e.target.value)} />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              ...
              <input type={showPassword ? "text" : "password"} required className="w-full h-[56px] bg-[#F9FAFB] border border-[#E5E7EB] rounded-2xl pl-12 pr-12 text-black font-semibold placeholder:text-[#D1D5DB] focus:border-[#C8952E] focus:bg-white outline-none transition-all" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] hover:text-[#C8952E] transition-colors">
                {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
              </button>
            </div>
          </div>

          <button type="submit" disabled={loading} className="w-full h-[60px] bg-[#C8952E] text-black font-black text-base rounded-2xl flex items-center justify-center hover:bg-[#D9A540] active:scale-[0.98] transition-all disabled:opacity-50 shadow-lg shadow-[#C8952E]/20 mt-4 uppercase tracking-widest">
            {loading ? <div className="w-6 h-6 border-3 border-black/20 border-t-black rounded-full animate-spin" /> : "Iniciar Sesión"}
          </button>
        </form>

        <div className="mt-10 pt-8 border-t border-[#F3F4F6] text-center">
          <p className="text-[11px] text-[#9CA3AF] font-bold uppercase tracking-widest">© 2026 PosVEN Pro · V 2.5.0</p>
        </div>
      </div>
    </div>
  );
